...
p2<-ggplot(data = try, aes_string(x = "Age",y = "est"))+
  ...
  scale_fill_manual(values=c("#6C80B6","#FF89ED","#21908C"))+scale_color_manual(values=c("#6C80B6","#FF89ED","#21908C"))+
  theme(legend.position = "None") + theme(plot.margin = margin(t = 0,  r = 0, b = 0, l = 0)) +ggtitle(plotnames[[i]]) +theme(text = element_text(family = "Arial"))+coord_cartesian(xlim = c(6, 25), expand = T)
...
ggsave('/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/tiledplots_CT_ASD_dup.pdf',tiledplots,width=20,height=10.5)
Sys.setenv(R_GSCMD = "/usr/local/bin/gs")
embed_fonts("/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/tiledplots_CT_ASD_dup.pdf")
dev.off()

tiledplots2<-(plots[[9]] | plots[[10]] | plots[[11]] | plots[[12]])/(plots[[13]]|plots[[14]]|plots[[15]]|plots[[16]]) + plot_annotation(title="Surface Area")
ggsave('/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/tiledplots_SA_ASD_dup.pdf',tiledplots2,width=20,height=10.5)
Sys.setenv(R_GSCMD = "/usr/local/bin/gs")
embed_fonts("/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/tiledplots_SA_ASD_dup.pdf")
dev.off()