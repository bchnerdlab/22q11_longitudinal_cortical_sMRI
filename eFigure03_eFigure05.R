...
lmod<-lm(adj_df[[var]]~Group+SEX+EstimatedTotalIntraCranialVol.combat,data=adj_df)
adj_df$yvar.res<-lmod$residuals
...
p2<-ggplot(data = try, aes_string(x = "Age",y = "est"))+
  ...
  geom_point(data = adj_df,aes_string(x = "Age", y = "yvar.res",color="Group"),size = 1,alpha=0.4)+
  geom_line(data = adj_df,aes_string(x = "Age", y = "yvar.res",group="PTID",color = "Group"),size = 0.2,alpha=0.4)+
  theme_bw()+
...
plots[[i]] <-  (p2/ (tile_con2/tile_del2/tile_dup2+plot_layout(guides='collect')) + plot_layout(nrow=2,heights=c(6,3))+
                  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())) +theme(text = element_text(family = "Arial"))
...
ggsave('/Users/jalbrzikowskime/Dropbox (BCH)/22q_dup_del_sMRI/results/tiledplots_CT_wpoints.pdf',tiledplots,width=20,height=10.5)

tiledplots2<-(plots[[9]] | plots[[10]] | plots[[11]] | plots[[12]])/(plots[[13]]|plots[[14]]|plots[[15]]|plots[[16]]) + plot_annotation(title="Surface Area")
ggsave('/Users/jalbrzikowskime/Dropbox (BCH)/22q_dup_del_sMRI/results/tiledplots_SA_wpoints.pdf',tiledplots2,width=20,height=10.5)
