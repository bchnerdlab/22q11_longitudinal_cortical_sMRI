...
```{r plot_pdf, include=F}

pdf("/Users/rahay/Box/22q_age_aftercombat_1.pdf",width=4.8,height=3.8)
for (i in c(165:203)){
  var=colnames(adj_df)[i]
  p<-ggplot(adj_df,aes(x=Age,y=adj_df[[i]])) + geom_point(aes(group=as.factor(SUBJECT_IDENTITY),color=as.factor(SUBJECT_IDENTITY)))+
    geom_line(aes(group=as.factor(PTID),color=as.factor(SUBJECT_IDENTITY)))+
                stat_smooth(aes(color=as.factor(SUBJECT_IDENTITY)),method="loess",span=1)+ylab(var)+ theme_bw()+theme(legend.position=c(.9,.75))
  print(p)
}
dev.off()
adj_df<-adj_df[adj_df$IQ_SCORE>0,]
ggplot(adj_df,aes(x=Age,y=adj_df$SUM.of.Pos.Symptoms)) + geom_point(aes(group=as.factor(SUBJECT_IDENTITY),color=as.factor(SUBJECT_IDENTITY)))+
    geom_line(aes(group=as.factor(PTID),color=as.factor(SUBJECT_IDENTITY)))+
                stat_smooth(aes(color=as.factor(SUBJECT_IDENTITY)),method="loess",span=1)+ theme_bw()+theme(legend.position=c(.9,.75))+theme(text = element_text(family = "Arial"))
...
write_xlsx(results_age_group_brain,path='/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/Aim1_age_results.xlsx')
