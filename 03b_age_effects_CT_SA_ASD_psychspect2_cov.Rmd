...
for (i in c(1:length(brain_regions))){
  ...
  mod<-gam(del22q[[var]]~s(Age,by=ASD.DIAGNOS,k=4)+
             ASD.DIAGNOS+SEX+SCANNER+EstimatedTotalIntraCranialVol.combat+psychosis_spect2+s(PTID,bs="re"),selection=TRUE,method="REML",data=del22q)
...
write_xlsx(results_age_ASD,path='/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/CT_SA_age_ASD_results_psychosis_spect2_cov.xlsx')
# 
#   for (i in c(1:length(brain_regions))){
# var=brain_regions[i]
#  name<-sprintf("/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/smooths_diff_%s_CT_ASD.pdf",var)
# pdf(name,height=5,width=8)
# mod<-gam(del22q[[var]]~s(Age,by=ASD.DIAGNOS)+ASD.DIAGNOS+SEX+SCANNER+EstimatedTotalIntraCranialVol.combat+s(PTID,bs="re"),selection=TRUE,method="REML",data=del22q)
#  
# plot(mod,pages=1)
# diff<-difference_smooths(mod,smooth="s(Age)")
# print(draw(diff))
