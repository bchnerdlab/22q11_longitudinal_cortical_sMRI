...
meds<-read_xlsx("/Users/rahay/Box/22q_dup_del_sMRI/data/REVISED_new_merged_data_WithSIPSratings_withMeds_06022021.xlsx")
...
for (i in c(1:length(brain_regions))){
  var=brain_regions[i]
  mod<-gam(adj_df[[var]]~s(Age,by=Group,k=4)+Group+SEX+s(PTID,bs="re",k=4)+EstimatedTotalIntraCranialVol.combat+IQ_SCORE,selection=TRUE,method="REML",data=adj_df)
...
#gap needs to be bigger than 0.44 to get a new range of ages 
...
write_xlsx(results_age_group_brain,path='/Users/jalbrzikowskime/Box/22q_dup_del_sMRI/results/Aim1_age_results_IQ.xlsx')
