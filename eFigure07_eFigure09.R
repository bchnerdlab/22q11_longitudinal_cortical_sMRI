...
for (i in c(1:length(brain_regions))){
  ...
  lmod<-lm(del22q[[var]]~psychosis_spect2+SEX+EstimatedTotalIntraCranialVol.combat,data=del22q)
...
p2<-ggplot(data = try, aes_string(x = "Age",y = "est"))+
  ...
  geom_point(data = del22q,aes_string(x = "Age", y = "yvar.res",color="psychosis_spect2"),size = 1,alpha=0.4)+
  geom_line(data = del22q,aes_string(x = "Age", y = "yvar.res",group="PTID",color = "psychosis_spect2"),size = 0.2,alpha=0.4)+
  theme_bw()+
 ...
 theme(plot.margin = margin(t = 0,  r = 0, b = 0, l = 0)) +ggtitle(plotnames[[i]]) +theme(text = element_text(family = "Arial"))+
  coord_cartesian(xlim = c(5, 50), expand = T)
...
ggsave('/Users/jalbrzikowskime/Dropbox (BCH)/22q_dup_del_sMRI/results/tiledplots_CT_psychosis_wpoints.pdf',tiledplots,width=20,height=10.5)
Sys.setenv(R_GSCMD = "/usr/local/bin/gs")
embed_fonts("/Users/jalbrzikowskime/Dropbox (BCH)/22q_dup_del_sMRI/results/tiledplots_CT_psychosis_wpoints.pdf")
dev.off()

tiledplots2<-(plots[[9]] | plots[[10]] | plots[[11]] | plots[[12]])/(plots[[13]]|plots[[14]]|plots[[15]]|plots[[16]]) + plot_annotation(title="Surface Area")
ggsave('/Users/jalbrzikowskime/Dropbox (BCH)/22q_dup_del_sMRI/results/tiledplots_SA_psychosis_wpoints.pdf',tiledplots2,width=20,height=10.5)
Sys.setenv(R_GSCMD = "/usr/local/bin/gs")
embed_fonts("/Users/jalbrzikowskime/Dropbox (BCH)/22q_dup_del_sMRI/results/tiledplots_SA_psychosis_wpoints.pdf")
dev.off()